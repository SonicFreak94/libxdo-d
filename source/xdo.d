...
import core.sys.posix.unistd : useconds_t;
import x11.X; 
import x11.Xlib;
...
/// ditto
enum SIZE_USEHINTS_X = (1L << 1);
/// ditto
enum SIZE_USEHINTS_Y = (1L << 2);
 ...
 */
struct charcodemap {
  wchar key; /** the letter for this key, like 'a' */
 ...
 */
enum SEARCH_TITLE = (1UL << 0);

/**
 * Search only window class.
 * @see xdo_search_windows
 */
enum SEARCH_CLASS = (1UL << 1);

/**
 * Search only window name.
 * @see xdo_search_windows
 */
enum SEARCH_NAME = (1UL << 2);

/**
 * Search only window pid.
 * @see xdo_search_windows
 */
enum SEARCH_PID = (1UL << 3);

/**
 * Search only visible windows.
 * @see xdo_search_windows
 */
enum SEARCH_ONLYVISIBLE = (1UL << 4);
 ...
 * @see xdo_search_windows
 */
enum SEARCH_SCREEN = (1UL << 5);

/**
 * Search only window class name.
 * @see xdo_search
 */
enum SEARCH_CLASSNAME = (1UL << 6);
...
enum SIZE_TO = 0;
enum SIZE_FROM = 1;
int xdo_wait_for_window_size(const xdo_t *xdo, Window window, uint width,
                             uint height, int flags, int to_or_from);
...
int xdo_translate_window_with_sizehint(const xdo_t *xdo, Window window,
                                       ...
                                       uint *width_ret, uint *height_ret);
 ...
 *
 * @param wid the window to query
 * @param width_ret pointer to uint where the width is stored.
 * @param height_ret pointer to uint where the height is stored.
 */
int xdo_get_window_size(const xdo_t *xdo, Window wid, uint *width_ret,
                        uint *height_ret);
...
int xdo_search_windows(const xdo_t *xdo, const xdo_search_t *search,
                      Window **windowlist_ret, uint *nwindows_ret);
 ...
 */
ubyte *xdo_get_window_property_by_atom(const xdo_t *xdo, Window window, Atom atom,
...
int xdo_get_window_property(const xdo_t *xdo, Window window, const char *property,
                            ubyte **value, long *nitems, Atom *type, int *size);
 ...
 */
uint xdo_get_input_state(const xdo_t *xdo);
...
int xdo_get_window_name(const xdo_t *xdo, Window window, 
                        ubyte **name_ret, int *name_len_ret,
                        int *name_type);
 ...
 */
int xdo_get_viewport_dimensions(xdo_t *xdo, uint *width,
                                uint *height, int screen);